...
namespace PubSub
{
    ...
    typedef std::string Message_Label_Name;
    typedef unsigned int Message_Label;
    class Message
    {
    public:
        Message(const Message_Label_Name str) : m_label(str) {}

        Message(const Message &obj) : m_label(obj.m_label) {}

        Message& operator=(const Message &obj) = default;

        Message_Label_Name getMessageLabelName() const { return m_label; };
